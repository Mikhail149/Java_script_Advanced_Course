...
class GoodsList {
  ...
  //Обработчик события кнопки "искать"
  clickFind(e) {
    //из поля ввода забираем значение
    let findText = document.querySelector('#find').value;
    //если значение равно нулю, то возвращаем полный список товаров и вызываем рендер
    if (findText.length == 0) {
      this.filtred = this.goods
      this.render();
      return
    }
    //создаем регулярное выражение из значения и ставим флаг игнорирования регистра
    const regexpFind = new RegExp(findText, `i`);
    //Перебираем основной массив товаров и сравниваем с регулрным выражением
    //Если естьсовпадение, то добавляем этот элемент в массив отфильтрованных товаров и вызываемрендер
    this.filtred = this.goods.filter((good) => regexpFind.test(good.product_name));
    if (this.filtred.length == 0) {
      this.filtred = [{ id_product: -1, product_name: '', price: '' }];
      this.render();
    } else {
      this.render();
    }
...
list.fetchGoods(() => {
  list.render();
  list.costGoods();
  document.querySelector('#butFind').addEventListener('click', list.clickFind.bind(list));
});
